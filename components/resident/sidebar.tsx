...
const navItems = [
  ...
  { href: '/schemes', label: 'Browse Schemes', icon: FileText },
  { href: '/ai-assistant', label: 'AI Assistant', icon: Bot },
  { href: '/nearby-offices', label: 'Nearby Offices', icon: MapPin },
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-5 border-b border-border">
        <Link href="/dashboard" className="flex items-center gap-3" onClick={() => setMobileOpen(false)}>
          ...
          <div>
            <p className="font-bold text-sm leading-tight">GramSeva AI</p>
            <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wider">Tamil Nadu Portal</p>
          </div>
        </Link>
      </div>
      ...
      {/* User card */}
      {profile && (
        <div className="p-4 mx-3 mt-3 rounded-xl bg-primary/5 border border-primary/10">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold truncate">{profile.name ?? 'Resident'}</p>
